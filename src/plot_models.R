...
combinedPlotROC <- function() {
  ...
  mapping <- c(
    "CNN-LSTM (Words)" = "rnn_words_random_embed", "CNN-LSTM (Words+Characters)" = "rnn_all_random_embed",
...
combinedPlot <- function() {
  ...
  g <- ggplot(data = store) +
    ...
    theme_bw() +
    ylim(c(min(store["precision"]), 1.00)) +
    xlim(c(min(store["recall"]), 1.00)) +
    xlab("\nRecall") +
    ylab("Precision\n") +
    theme(
      text = element_text(size = 13, family = "CM Roman"),
...
svmWordPlot <- function() {
  ...
  g <- ggplot(data = spam_words, aes(reorder(word, coefficient, sum), coefficient)) +
    geom_col(
    ...
    theme_bw() +
    theme(
      text = element_text(size = 16, family = "CM Roman"), legend.position = "none", plot.title = element_text(hjust = 0.5),
      axis.title.y = element_text(margin = margin(t = 0, r = 20, b = 0, l = 0)),
      axis.title.x = element_text(margin = margin(t = 20, r = 0, b = 0, l = 0))
    ) +
    xlab("Word feature") +
    ylab(TeX("Absolute weight |$\\theta$|")) +
  ...
  g <- ggplot(data = ham_words, aes(reorder(word, coefficient, sum), coefficient)) +
    geom_col(
...
option_list <- list(
  make_option(c("-t", "--type"),
    type = "character", default = "combined",
