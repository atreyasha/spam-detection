...
def blindRNN(pickle_file,
    ...
    if "words" in pickle_file or "all" in pickle_file:
        ...
        X_blind_words = pad_sequences(
            [[word_dict[el] if el in word_dict.keys() else 0 for el in ls]
             for ls in blind_data],
            maxlen=maxlen_words)
    if "char" in pickle_file or "all" in pickle_file:
        with open("./data/rnn/char/integer_index_char.pickle", "rb") as f:
            char_dict = pickle.load(f)
        X_blind_char = pad_sequences([[
            char_dict[el.lower()] if el.lower() in char_dict.keys() else 0
            for el in ls
        ] for ls in text],
                                     maxlen=maxlen_char)
    model = keras.models.load_model(
        glob("./pickles/" + pickle_file + "/best*")[0])
...
def blindSVM(pickle_file, text, y_blind):
    ...
    if "rbf" in pickle_file:
        number = int(
            re.sub(".pickle", "", re.sub(r".*best_model_", "", full_name)))
...
if __name__ == "__main__":
    ...
    if args.pickle != "all":
        if "rnn" in args.pickle:
            ...
            blindRNN(args.pickle, blind_data, text, y_blind,
                     args.padding_tokens, args.padding_char)
        ...
        for file in files:
            ...
            if "rnn" in file:
                ...
                blindRNN(filename, blind_data, text, y_blind,
                         args.padding_tokens, args.padding_char)
            elif "svm" in file:
