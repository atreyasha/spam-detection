...
def charEmbeddingGen():
    ...
    for file in tqdm(file_path):
        ...
        base_name = re.sub(
            ".txt", "",
            os.path.dirname(file) + "/" + os.path.basename(file)) + '_char.txt'
        with open(base_name, 'w') as f:
            for word in vectors:
                avg_vector = np.round((vectors[word][0] / vectors[word][1]),
                                      6).tolist()
                f.write(word + " " + " ".join(str(x)
                                              for x in avg_vector) + "\n")
...
def embeddingMatrix():
    ...
    for file in file_path:
        ...
        embedding_matrix = np.zeros(
            (len(index.keys()) + 1, embedding_vector_size))
